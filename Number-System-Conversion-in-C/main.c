    ...
    while(numbers!=0)
    {
        ...
        switch(operations)
        {
            case 1:
                ...
                checking=b; // Kiểm tra số nhị phân
                ...
                while(checking!=0)
                {
                    ...
                    if(numbers>1)
                    {
                        ...
                        printf("***TRY AGAIN****\n");
                        goto D; // không phải nhị phân thì yêu cầu nhập lại số
                    }
                    else
            ...
            case 2:
                ...
                while(checking!=0) // kiểm tra số nhị phân
                {
                    numbers=checking%10;
                    if(numbers>1)
                    {
                        printf("\n%d IS NOT BINARY NUMBER.\n",b);
                        printf("***TRY AGAIN****\n");
                        goto E; // sai thì nhập lại
                    }
                    else // đúng thì thực hiện
                    checking=checking/10; //lấy số
                }
...
long int Binary_to_Octal(long int bin)
{
    ...
    x=0;
    while(sum!=0) //decimal chuyển về octal thì chia 8 cho đến khi bằng 0 rồi lấy dãy số dư từ dưới lên
    {
        remaining[x]=sum%8;
        sum=sum/8;
        x++;
